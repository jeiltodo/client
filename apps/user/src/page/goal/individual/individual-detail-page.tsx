...
import { useDeleteSingleGoal, useEditSingleGoal } from '../../../entities/user/hooks/useSingleGoalMuate';
...
export const IndivDetailPage = ({ params }: { params: { id: number } }) => {
	...
	const goalsForModal = goals?.map((goal) => ({ id: goal.id, title: goal.title })) ?? [];
	...
	return (
		<div>
			{!isLoading && singleGoal && singleGoalTodo && (
				<>
					<LayoutTitle title={`${decodeURIComponent(user?.nickname ?? '개인')}의 목표`} />
					<div className='flex flex-col gap-y-6'>
						<TitleProgressBarCard goalData={singleGoal} onEditGoal={openEditModal} onDeleteGoal={openDeleteModal} />
						<NotesPushButton goalData={singleGoal} />
						<Button variant='text-blue' className='flex gap-1 items-center text-sm h-[20px]' onClick={openAddTodoModal}>
						...
						{isConfirmOpen && (
							<ConfirmationModal setModalToggle={setIsConfirmOpen} submitButtonText={'삭제'} onSubmit={handleDelete}>
								정말 삭제 하시겠어요?
							</ConfirmationModal>
						)}
						{isAddTodoModalOpen && (
