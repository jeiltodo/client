...
import { BoardTitle, Button, ButtonGroup, LayoutTitle, TodoTitle } from '@jeiltodo/ui/shared';
	...
	return (
		<>
			<LayoutTitle title={`노트 ${noteid === 'new' ? '작성' : '수정'}`}>
				<ButtonGroup>
					<Button className='w-[96px]' onClick={handleLocalSave} variant='text-blue'>
						임시저장
					</Button>
					<Button
						className='w-[96px]'
			...
			<BoardTitle className='mb-[12px]' icon='flag' iconSize={24} title='자바스크립트로 웹 서비스 만들기' />
			<TodoTitle className='mb-[24px]' title='자바스크립트 기초 챕터1 듣기' />
			<EditorForm content={content} setContent={setContent} setTitle={setTitle} title={title} />
		</>
	);
};
