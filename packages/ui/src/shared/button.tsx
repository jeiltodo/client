...
export const Button = ({ 
  ...
  const baseClasses = 'font-pretendard-medium text-base h-12 transition-all duration-300 ease-in-out min-w-[120px]';
  ...
  const variantClasses = {
    ...
    'warning': 'bg-transparent text-blue-500 border border-blue-500 rounded-xl hover:bg-blue-500 hover:text-white',
    'error': 'bg-transparent text-blue-500 border border-blue-500 rounded-xl hover:bg-blue-500 hover:text-white',
  };
...
// `.trim().replace(/\s+/g, ' ');
  ...
  return (
    <button className={buttonClasses} {...props} style={{ outline: 'none', boxShadow: 'none' }} onClick={onClick ? onClick : voidFn} disabled={isDisabled}>
      {children}
    </button>
  );
};
